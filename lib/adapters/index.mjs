...
export async function publish(client, articles, allPublished = []) {
  ...
  switch (client.publish_adapter) {
    case 'github': {
      ...
      // Lote vazio é uso legítimo: regerar índice e sitemap depois de um
      // conserto no banco, sem reescrever artigo nenhum.
      const message = articles.length
        ? `content: ${articles.map((a) => a.slug).join(', ')}`
        : 'content: regera índice e sitemap';
