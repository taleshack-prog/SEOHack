...
// Conferidos no console da Anthropic em 20/08/2026. A versão anterior destes
// números era chute e superestimava o Sonnet 5 em 50%.
const PRICES = {
...
/**
 * @returns {Promise<{id:number, cost:number}>}
 *
 * Devolve o id porque a chamada acontece ANTES de o artigo existir no banco —
 * o esboço e o rascunho são gerados para depois virar linha em `articles`. Sem
 * o id para amarrar depois, `article_id` ficava nulo para sempre e o custo por
 * artigo aparecia como US$ 0,00 no painel enquanto o total subia.
 */
...
/** Amarra chamadas já registradas ao artigo que elas produziram. */
export async function linkUsage(clientId, articleId, ids) {
  if (!ids?.length) return 0;
  const r = await sql`
    UPDATE llm_usage SET article_id = ${articleId}
     WHERE client_id = ${clientId} AND id = ANY(${ids}) AND article_id IS NULL
    RETURNING id`;
  return r.length;
}
