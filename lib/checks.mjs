...
/**
 * O prompt está empacotado junto com a função?
 *
 * A Vercel só inclui no bundle o que detecta por análise estática do código.
 * `readFile` com caminho montado em runtime não é detectado, então o
 * system-prompt ficava de fora e a função quebrava com ENOENT — depois de já
 * ter passado por autenticação, orçamento e seleção de tópicos. Um arquivo de
 * texto ausente derrubava o pipeline no meio.
 *
 * O `includeFiles` no vercel.json resolve; esta checagem garante que a gente
 * descubra antes de gastar tokens, e não durante a geração.
 */
export async function checkPrompt(nome = 'system-prompt-v5.md') {
  try {
    const { readFile } = await import('node:fs/promises');
    const { fileURLToPath } = await import('node:url');
    const caminho = fileURLToPath(new URL(`../prompts/${nome}`, import.meta.url));
    const texto = await readFile(caminho, 'utf8');
    if (texto.trim().length < 200) {
      return { ok: false, reason: `prompt "${nome}" existe mas está quase vazio` };
    }
    return { ok: true, prompt: nome, bytes: texto.length };
  } catch (err) {
    return { ok: false, prompt: nome,
      reason: err.code === 'ENOENT'
        ? `prompt "${nome}" não foi empacotado — confira includeFiles no vercel.json`
        : err.message };
  }
}

/**
 * Testa a Search Console de verdade.
 *
 * Ter as três variáveis preenchidas não prova nada: a service account precisa
 * ter sido adicionada como usuária DA PROPRIEDADE, num painel diferente do
 * Google Cloud onde ela foi criada. É o passo mais esquecido da configuração,
 * e o sintoma é um 403 genérico — que sem esta checagem só apareceria quando o
 * cron de tracking falhasse, dias depois.
 */
export async function checkGsc() {
  const props = ['GSC_CLIENT_EMAIL', 'GSC_PRIVATE_KEY', 'GSC_PROPERTY'];
  const faltando = props.filter((k) => !process.env[k]);
  if (faltando.length) return { ok: false, skipped: true, reason: `faltando: ${faltando.join(', ')}` };

  try {
    const { defaultWindow, byPage } = await import('./search-console.mjs');
    const janela = defaultWindow(3);
    const linhas = await byPage(janela, 5);
    return {
      ok: true,
      property: process.env.GSC_PROPERTY,
      window: `${janela.startDate} a ${janela.endDate}`,
      rows: linhas.length,
      // Zero linhas não é erro: o dado tem 2-3 dias de atraso e site novo
      // ainda não acumulou impressão.
      note: linhas.length ? null : 'autenticou, mas ainda sem dados nesta janela',
    };
  } catch (err) {
    const m = err.message || '';
    const reason = /403/.test(m)
      ? 'a service account não tem acesso à propriedade — adicione o client_email '
        + 'em Search Console → Configurações → Usuários e permissões, com permissão Completo'
      : /404/.test(m)
        ? `propriedade "${process.env.GSC_PROPERTY}" não encontrada — confira o formato `
          + '(sc-domain:dominio.com.br para propriedade de Domínio)'
        : /invalid_grant|JWT|PEM|DECODER/i.test(m)
          ? 'chave privada malformada — as quebras de linha devem estar como \\n literais'
          : err.quota ? 'quota da GSC excedida — tente em 15 minutos'
            : 'falha ao consultar a Search Console';
    return { ok: false, reason, googleSaid: m.slice(0, 300) };
  }
}
