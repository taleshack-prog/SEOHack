...
import { assertBudget, recordUsage, linkUsage } from './budget.mjs';
import { validateArticle, productForCluster } from './validate.mjs';
import { parseNotes } from './notes.mjs';
import { pillarsToRefresh } from './cluster-links.mjs';
...
Gere APENAS um objeto JSON, sem cercas de markdown e sem preâmbulo:
{
  ...
  const uso = await recordUsage(clientId, { stage: 'outline', provider: r.provider, model: r.model,
    inputTokens: r.inputTokens, outputTokens: r.outputTokens });
  const outline = parseJson(r.text, r.stopReason);
  outline._usageId = uso.id;
  return outline;
}
...
frontmatter, não inclua o H1 (o título vem do frontmatter), não use HTML bruto.`,
  ...
  const uso = await recordUsage(clientId, { stage: 'draft', provider: r.provider, model: r.model,
    inputTokens: r.inputTokens, outputTokens: r.outputTokens });
  return { markdown: r.text.trim(), model: r.model, provider: r.provider, usageId: uso.id };
}
 ...
 */
function buildContext(cfg, publicados, clusterAtual = null) {
  // Caminho sem descrição não diz ao modelo QUANDO linkar. Com "/genbreed"
  // numa lista de caminhos, um artigo de genética não teria como saber que ali
  // está o simulador. Produtos cadastrados vêm com nome, descrição e cluster.
  const cadastrados = cfg.products || [];
  const doClusterProd = productForCluster(cadastrados, clusterAtual);
  const soCaminhos = (cfg.productPaths || [])
    .filter((c) => !cadastrados.some((p) => p.path === c));
  const produtos = [
    ...cadastrados.map((p) => `- ${p.path} — ${p.name}: ${p.about}`
      + (p === doClusterProd ? '  ← PRODUTO DESTE CLUSTER' : '')),
    ...soCaminhos.map((c) => `- ${c}`),
  ].join('\n');
  const regraProduto = doClusterProd
    ? `\nEste artigo é do cluster "${clusterAtual}", criado para levar leitores a `
      + `${doClusterProd.name}. Linke para ${doClusterProd.path} pelo menos uma vez, `
      + 'no ponto em que o leitor ganharia algo experimentando o produto — por exemplo, '
      + 'depois de explicar um conceito que ele permite praticar. Uma menção natural; '
      + 'não transforme o artigo em anúncio. O artigo precisa ser útil para quem nunca '
      + 'vai clicar no link.\n'
    : '';
  // A persona do prompt é "engenheiro de software escrevendo para devs". Para
  // um cluster de genética isso sairia errado no tom e nas fontes. O produto
  // pode trazer a própria voz, que vale só para os clusters dele.
  const voz = doClusterProd?.voice
    ? `\n## Voz e público deste artigo (substitui a persona do início do prompt)\n${doClusterProd.voice}\n`
      + 'As proibições absolutas e a estrutura obrigatória continuam valendo. Na regra de '
      + 'entidades, use as entidades do tema deste artigo, não as de SaaS ou Web3.\n'
    : '';
  const blog = cfg.blogBasePath || '/blog';

  // Agrupado por cluster, e o cluster do artigo em produção vem primeiro.
  //
  // A versão anterior entregava uma lista plana. O modelo então linkava para
  // qualquer artigo publicado, e o resultado no blog real foi o pilar de web3 e
  // um artigo de automação apontando para o pilar de saas. Com cinco artigos é
  // ruído; com trinta, dilui exatamente o sinal que a estratégia de clusters
  // do PRD §34 depende — profundidade concentrada num tema.
  const porCluster = new Map();
  for (const a of publicados) {
    const c = a.cluster || 'sem cluster';
    if (!porCluster.has(c)) porCluster.set(c, []);
    porCluster.get(c).push(a);
  }

  const ordem = [...porCluster.keys()].sort((x, y) =>
    (x === clusterAtual ? -1 : 0) - (y === clusterAtual ? -1 : 0));

  const listaArtigos = ordem.length
    ? ordem.map((c) => {
        const marca = c === clusterAtual ? ' ← ESTE É O CLUSTER DO SEU ARTIGO' : '';
        const itens = porCluster.get(c)
          .map((a) => `  - ${blog}/${a.slug} — "${a.title}"${a.is_pillar ? ' (PÁGINA PILAR)' : ''}`)
          .join('\n');
        return `Cluster "${c}"${marca}\n${itens}`;
      }).join('\n\n')
    : '(nenhum artigo publicado ainda)';

  const doCluster = publicados.filter((a) => a.cluster === clusterAtual);

  const regraInterna = publicados.length === 0
    ...
    : doCluster.length >= 2
      ? `Linke para pelo menos 2 artigos do SEU cluster ("${clusterAtual}"), incluindo a página pilar. ` +
        'Só linke para artigo de outro cluster se a conexão for realmente forte — ' +
        'link cruzando cluster dilui a autoridade temática.'
      : doCluster.length === 1
        ? `Há um artigo publicado no seu cluster ("${clusterAtual}"). Linke para ele. ` +
          'Se precisar de um segundo link, escolha o mais próximo tematicamente.'
        : 'Ainda não há artigos publicados no seu cluster. Use no máximo um link ' +
          'para outro cluster, e só se a conexão for direta.';
...
link quebrado.
${regraProduto}${voz}
## Artigos já publicados, por cluster
${listaArtigos}
...
export async function generateBatch(client, { topicIds = [], limite = MAX_PER_RUN } = {}) {
    ...
    const publicados = await sql`
      SELECT id, slug, title, description, cluster, is_pillar, markdown,
             first_published_at, frontmatter
        FROM articles
       WHERE client_id = ${client.id} AND status = 'published'
       ORDER BY first_published_at ASC`;

    const promptBase = await loadPrompt('system-prompt-v5.md');
    const batch = [];
    const outcomes = [];
    ...
    for (const topic of topics) {
      try {
        ...
        // Contexto montado por tópico: o cluster do artigo muda a orientação
        // de links internos.
        const system = promptBase
          + buildContext(client.adapter_config || {}, publicados, topic.cluster);
        ...
        const { markdown, model, provider, usageId } = await generateArticle(topic, outline, system, client.id);
        ...
        const check = validateArticle({ slug, frontmatter, markdown },
          { ...client.adapter_config, existingSlugs: publicados.map((a) => a.slug),
            ...
            clusterSlugs: topic.cluster
              ? publicados.filter((a) => a.cluster === topic.cluster).map((a) => a.slug)
              : undefined });
        if (!check.valid) {
          // Só o nome da regra não permite julgar se o problema é do texto ou
          // da regra. "unsourced_statistic" não diz qual número foi acusado —
          // e sem isso não dá para saber se o validador está agressivo demais.
          throw new Error('validação local: '
            + check.errors.map((e) => `${e.rule} (${e.detail})`).join(' | '));
        }
        for (const w of check.warnings) console.warn(`[content] aviso ${slug}: ${w.rule}`);

        // Número sem fonte não descarta o artigo — segura para revisão. O
        // texto já foi pago; quem decide se "100%" é estatística é o operador,
        // não uma regex. A tela de revisão mostra os trechos destacados.
        const conferir = check.review.length > 0;
        const segurar = needsHuman || conferir;
        frontmatter.draft = segurar;
        if (conferir) {
          console.warn(`[content] ${slug} segurado para revisão: `
            + check.review.map((r) => r.detail).join(' | '));
        }
        ...
        const [article] = await sql`
          ...
          VALUES (${client.id}, ${topic.id}, ${slug}, ${outline.title}, ${outline.description},
                  ...
                  ${segurar ? 'needs_human' : 'ready'}, ${needsHuman}, ${provider}, ${model},
        ...
        await linkUsage(client.id, article.id, [outline._usageId, usageId].filter(Boolean));

        batch.push({ slug, markdown, frontmatter, _articleId: article.id, _topicId: topic.id });
        outcomes.push({ slug, needsHuman: segurar, conferir, words: check.stats.words });
      } catch (err) {
    ...
    for (const item of prontos) {
      ...
      if (rejected) {
        ...
        // status = 'published' é o que estava faltando, e o buraco era grande:
        // o artigo ia para o site, mas continuava 'ready' no banco. Tudo que
        // consulta `status = 'published'` — a lista do painel, a tela de
        // desempenho, o índice do blog, o sitemap e a lista de slugs que o
        // validador usa para links internos — passava ao largo dele. Cinco
        // artigos ficaram no ar sem estar em lugar nenhum: sem link interno,
        // fora do sitemap, invisíveis para o operador. Só quem tivesse a URL
        // exata chegava lá.
        await sql`
          UPDATE articles
             SET status = ${isDraft ? 'needs_human' : 'published'},
    ...
    // --- links de volta no pilar (PRD §34) ---
    //
    // Roda DEPOIS da publicação e num segundo commit, de propósito: o pilar só
    // pode linkar para o satélite depois que o satélite existe. Não chama o
    // LLM — é montagem de lista a partir do que está no banco, custo zero.
    let pilaresAtualizados = [];
    try {
      ...
      const site = client.adapter_config || {};
      const pendentes = pillarsToRefresh(agora, result.committed, site);
      ...
      if (pendentes.length) {
        ...
        const r2 = await publishViaAdapter(client, lote, agora);

        for (const item of lote) {
          if (!r2.committed.includes(item.slug)) continue;
          await sql`
            UPDATE articles
        ...
        pilaresAtualizados = r2.committed;
      }
    } catch (err) {
      // Falhar aqui não invalida a publicação do satélite, que já aconteceu.
      console.error('[content] links de cluster não atualizados:', err.message);
    }
    ...
    return {
      ...
      pillarsUpdated: pilaresAtualizados,
...
export { MAX_PER_RUN, buildContext };
