...
export function extractRequest(log) {
  ...
  return {
    ...
    // O host decide de qual cliente é a visita. Um único drain pode receber
    // logs de vários projetos, e sem isto todo hit caía no cliente padrão.
    host: String(p.host || log?.host || '').toLowerCase().replace(/^www\./, ''),
...
export function aggregate(logs) {
  ...
  for (const log of logs) {
    const { userAgent, host, path, statusCode, timestamp } = extractRequest(log);
    const agente = identifyCrawler(userAgent);
    if (!agente) continue;
    const data = new Date(Number(timestamp) || Date.now()).toISOString().slice(0, 10);
    const chave = `${host}|${agente}|${path}|${data}`;
    const atual = mapa.get(chave);
    if (atual) atual.hits += 1;
    else mapa.set(chave, { host, agente, path, statusCode, data, hits: 1 });
  }
  return [...mapa.values()];
}
