...
async function query(body, opts = {}) {
  const { maxAttempts = 5 } = opts;
  // A propriedade vem do cliente. O fallback para GSC_PROPERTY mantém quem
  // ainda roda com um site só funcionando sem mexer em variável de ambiente.
  const property = encodeURIComponent(opts.property || process.env.GSC_PROPERTY);
...
export async function byPage({ startDate, endDate } = defaultWindow(), rowLimit = 1000, opts = {}) {
