...
import { checkCodeApis } from './code-check.mjs';
...
// Marcadores de exemplo hipotético e de contexto de cálculo. Parágrafo com
// qualquer um deles é aritmética de ensino, não afirmação sobre o mundo.
...
// Proporção: "R$ 3 para cada R$ 1", "LTV:CAC de 3:1", "a razão entre".
const PROPORCAO = /\bcada\s+(?:R|US)\$|\b(para cada|a cada|propor[çc][ãa]o|raz[ãa]o|rela[çc][ãa]o (entre|de))\b|\b\d+\s?:\s?\d+\b/i;
// 0% e 100% são limites, não medições: "NRR acima de 100%", "reúne 100% das
// frações", "0% de churn". Nenhuma pesquisa é necessária para citá-los.
const LIMITE = /^(0|100)(?:[.,]0+)?\s?%$/;

/**
 * Números com cara de estatística que aparecem sem link de fonte no mesmo
 * parágrafo. Não é veredito — é a lista do que o operador precisa conferir.
 * @returns {Array<{numero:string, trecho:string}>}
 */
export function findUnsourcedStats(markdown = '') {
  const out = [];
  for (const par of markdown.split(/\n{2,}/)) {
    const t = par.trimStart();
    if (t.startsWith('```') || t.startsWith('|') || t.startsWith('    ')) continue;
    if (EXEMPLO.test(par) || CALCULO.test(par) || PROPORCAO.test(par)) continue;

    STAT_FORTE.lastIndex = 0;
    STAT_FRACA.lastIndex = 0;
    const fortes = (par.match(STAT_FORTE) || []).filter((n) => !LIMITE.test(n.trim()));
    const fracas = AFIRMACAO.test(par) ? (par.match(STAT_FRACA) || []) : [];
    const achados = [...fortes, ...fracas];
    if (!achados.length) continue;

    const hasSource = [...par.matchAll(LINK_RE)].some((m) => /^https?:\/\//i.test(m[1]));
    if (!hasSource) out.push({ numero: achados[0].trim(), trecho: par.trim() });
  }
  return out;
}

// O link aponta para o destino, ou para algo DENTRO dele. Prefixo simples
// aceitava "https://genbreed.com.br.golpe.com" como link para o GenBreed.
function apontaPara(href, destino) {
  if (!destino) return false;
  const base = destino.replace(/\/+$/, '');
  if (href === base || href === `${base}/`) return true;
  return ['/', '?', '#'].some((sep) => href.startsWith(base + sep));
}

/**
 * Produto associado a um cluster, se houver.
 * @param {Array<{path:string,name:string,clusters?:string[]}>} products
 */
export function productForCluster(products = [], cluster = null) {
  if (!cluster || !Array.isArray(products)) return null;
  return products.find((p) => (p.clusters || []).includes(cluster)) || null;
}

/**
 * @param {{slug:string, frontmatter:object, markdown:string}} article
 * @returns {{valid:boolean, errors:Array<{rule:string,detail:string}>, review:Array<{rule:string,detail:string,trechos:Array}>, warnings:Array, stats:object}}
 */
export function validateArticle({ slug, frontmatter = {}, markdown = '' }, site = {}) {
  // Produtos cadastrados (site.products) também contam como destino de produto,
  // inclusive os de domínio próprio, como https://genbreed.com.br.
  const productPaths = [...new Set([
    ...(site.productPaths || DEFAULT_PRODUCT_PATHS),
    ...(site.products || []).map((p) => p.path).filter(Boolean),
  ])];
  const produtoDoCluster = productForCluster(site.products, site.cluster);
  ...
  const product = links.filter((h) => productPaths.some((p) => apontaPara(h, p)));
  ...
  //
  // Com o cluster informado, conta só os artigos DO cluster. Sem isso, o
  // primeiro artigo de um cluster novo (genética, num blog de SaaS e Web3) era
  // obrigado a linkar dois artigos de outro tema — o contexto do modelo manda
  // o contrário, e o artigo seria descartado depois de pago.
  const base = Array.isArray(site.clusterSlugs) ? site.clusterSlugs : existentes;
  const minInternos = base === null
    ? LIMITS.minInternalLinks
    : Math.min(LIMITS.minInternalLinks, base.length);
  ...
  // Cluster criado para levar tráfego a um produto: o link para ESSE produto é
  // obrigatório. Sem isto, um artigo de genética podia cumprir a regra acima
  // linkando /contato — e o produto que motivou o cluster ficava sem visita.
  if (produtoDoCluster && !links.some((h) => apontaPara(h, produtoDoCluster.path))) {
    fail('cluster_product_link',
      `artigo do cluster "${site.cluster}" sem link para ${produtoDoCluster.name} (${produtoDoCluster.path})`);
  }

  // --- integridade factual (PRD §8.1) ---
  //
  // Número sem fonte NÃO reprova mais o artigo: vai para `review`, e o artigo
  // entra na fila de revisão com os trechos destacados. O operador decide.
  //
  // Motivo: a detecção é por padrão de texto e não sabe distinguir "73% das
  // startups falham" (inventado) de "NRR acima de 100%" (limiar) ou "R$ 3 de
  // LTV para cada R$ 1 de CAC" (proporção). Quatro lotes seguidos tiveram
  // artigos legítimos descartados depois de escritos e pagos. Descartar é
  // caro e irreversível; pedir um olhar humano é barato.
  const review = [];
  const suspeitos = findUnsourcedStats(markdown);
  if (suspeitos.length) {
    review.push({
      rule: 'unsourced_statistic',
      detail: `${suspeitos.length === 1 ? 'número' : 'números'} sem link de fonte: `
        + suspeitos.slice(0, 3).map((x) => x.numero).join(', '),
      trechos: suspeitos,
    });
  }

  // --- código de exemplo (PRD §8.1 estendido) ---
  //
  // A regra de integridade factual cobria estatística, link e experiência, mas
  // não código. Um artigo sobre Vercel saiu publicado com `caches.default`, que
  // é API do Cloudflare e lança TypeError na primeira execução. Quem copiar
  // perde uma hora, e o custo cai no domínio inteiro.
  for (const achado of checkCodeApis(markdown)) {
    fail('wrong_platform_api', achado.detail);
  }
